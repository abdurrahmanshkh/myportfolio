<script>
	import {AccordionItem} from 'flowbite-svelte';
</script>
 
<div class="border-b-2 border-gray-300 dark:border-gray-700" id="about">
	<AccordionItem class="pt-0" open>
		<span slot="header" class="text-2xl font-semibold text-gray-900 dark:text-gray-100">About</span>
		<div class="mb-1 font-semibold text-gray-700 dark:text-gray-300">
			Enthusiastic and dedicated engineering student with proficiency in Python, C, Go, and Java.
			Experienced in full stack development and project management. Proven ability in leading
			teams and delivering innovative projects, such as DevStar and MapMyProject. Seeking to leverage
			technical skills and passion for software development in a dynamic industry setting.
		</div>
	</AccordionItem>
</div>